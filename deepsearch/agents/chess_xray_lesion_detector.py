...
def predict(image_path: str) -> PredictionResult:
    ...
    return PredictionResult(
        xyxyn=results.xyxyn.cpu().numpy().tolist(),
        conf=results.conf.cpu().numpy().tolist(),
        cls=results.cls.cpu().numpy().astype(int).tolist(),
        org_size=size, 
        org_path=image_path,
    )
...
def xray_dianose_agent(img_path: str) -> tuple[Optional[np.ndarray], Optional[str]]:
    client = OpenAI(
